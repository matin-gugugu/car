...
def rental_order_list():
    ...
    cursor = db.execute(
        ...
        SELECT id, plate, car_type AS carType, driver_name AS driverName,
               driver_phone AS driverPhone, start_date AS startDate,
               end_date AS endDate, deposit, status, remark
        FROM rental_orders
        """
        + where_sql
        + " ORDER BY id ASC",
...
def rental_order_create():
    ...
    cursor = db.execute(
        """
        INSERT INTO rental_orders (plate, car_type, driver_name, driver_phone, start_date, end_date, deposit, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            payload["plate"],
